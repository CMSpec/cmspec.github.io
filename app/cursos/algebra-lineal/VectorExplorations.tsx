...
type Plot = { cx: number; cy: number; scale: number };

const u: Vector2 = [2, 1];
const v: Vector2 = [-1, 2];
const centeredPlot: Plot = { cx: 260, cy: 165, scale: 62 };
const loweredPlot: Plot = {
  ...centeredPlot,
  cy: centeredPlot.cy + 1.5 * centeredPlot.scale,
};

function point([x, y]: Vector2, plot = centeredPlot) {
...
function VectorArrow({
  ...
  const start = point(from, plot);
  const end = point(to, plot);
  return (
...
function Plane({ children, label, plot = centeredPlot }: { children: React.ReactNode; label: string; plot?: Plot }) {
  ...
  return (
    <svg className="vector-plane" viewBox="0 0 520 330" role="img" aria-label={label}>
      ...
      <g className="vector-grid">
        {grid.map((value) => {
          const vertical = point([value, 0], plot).x;
          const horizontal = point([0, value], plot).y;
          return (
            <g key={value}>
...
export function VectorSumLab() {
  ...
  return (
    <section className="vector-lab" aria-labelledby="vector-sum-title">
      ...
      <Plane label="Suma geométrica de los vectores u y v mediante la regla punta con cola" plot={loweredPlot}>
        <VectorArrow className={`vector-arrow vector-arrow-u ${step === 0 ? "is-current" : ""}`} to={u} label="u" plot={loweredPlot} />
        <VectorArrow className={`vector-arrow vector-arrow-v ${step === 1 ? "is-current" : ""}`} from={u} to={result} label="v" plot={loweredPlot} />
        <VectorArrow className={`vector-arrow vector-arrow-sum ${step === 2 ? "is-current" : ""}`} to={result} label="u + v" plot={loweredPlot} />
        <line className="vector-guide" x1={point(v, loweredPlot).x} y1={point(v, loweredPlot).y} x2={point(result, loweredPlot).x} y2={point(result, loweredPlot).y} />
        <VectorArrow className="vector-arrow vector-arrow-v vector-arrow-ghost" to={v} label="v" plot={loweredPlot} />
      </Plane>
...
export function VectorCombinationLab() {
  ...
  const corners = [[0, 0], u, add(u, v), v].map((value) => point(value as Vector2, loweredPlot));
  ...
  return (
    <section className="vector-lab" aria-labelledby="vector-combination-title">
      ...
      <Plane label="Rombo formado por las combinaciones alfa u más beta v con alfa y beta entre cero y uno" plot={loweredPlot}>
        <polygon className="vector-span-region" points={corners.map(({ x, y }) => `${x},${y}`).join(" ")} />
        {samples.map((sample, index) => {
          const samplePoint = point(sample.value, loweredPlot);
        ...
        <VectorArrow className="vector-arrow vector-arrow-u" to={u} label="u" plot={loweredPlot} />
        <VectorArrow className="vector-arrow vector-arrow-v" to={v} label="v" plot={loweredPlot} />
        <circle className="combination-point" cx={point(combination, loweredPlot).x} cy={point(combination, loweredPlot).y} r="7" />
      </Plane>
      <div className="vector-lab-controls">
