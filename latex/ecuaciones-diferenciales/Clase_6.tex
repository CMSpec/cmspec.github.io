...
\begin{center}
$e^{(\alpha + \beta i)x} =e^{\alpha x}\cos(\beta x)+ e^{\alpha}\sin(\beta x)i$
\end{center}

\begin{center}
$e^{(\alpha - \beta i)x} = e^{\alpha x}\cos(\beta x) - e^{\alpha x}\sin(\beta x)i$
\end{center}
...
\begin{center}
$y_2 = \displaystyle \frac{e^{(\alpha + \beta i)x}- e^{(\alpha - \beta i)x} }{2i} = e^{\alpha x}\sin(\beta x)$
\end{center}
...
\begin{ejem}Resolver la ecuación diferencial $y'' + y' -6y =0$ \\
\end{ejem}

\begin{sol}sustituyendo por $e^{mx}$, obtenemos
\end{sol}
...
\begin{ejem}Resolver la ecuación diferencial $y'' + w^2 y=0$ donde $w$ es una constante \\
\end{ejem}
...
\begin{center}
$y_2 = e^{0x} \sin(\omega x) = \sin(\omega x)$
\end{center}
...
\begin{align*}
  \frac{dy}{dx} &= -\frac{y\cos(x)+2x}{\sin(x)+\cos(y)}
\end{align*}


\begin{sol}Reescribiendo la ecuación
\end{sol}

\begin{align*}
  (y\cos(x)+2x) dx+(\sin(x)+\cos(y) )dy &= 0
\end{align*}
Derivando parcialmente
\begin{align*}
  \frac{\partial }{\partial x}(\sin(x)+\cos(y)) &= \cos(x) =\frac{\partial }{\partial y}(y\cos(x)+2x),
\end{align*}
verificamos que la ecuaci\'on es exacta. Integrando
\begin{align*}
  f(x,y)=\int y\cos(x)+2x dx&= y\sin(x)+x^2+c(y)
\end{align*}
para encontrar $c(y)$
\begin{align*}
  \frac{\partial }{\partial y} ( y\sin(x)+x^2+c(y))&=\sin(x)+c'(y)= \sin(x)+\cos(y)\\
  c'(y)&=\cos(y)\\
    c(y)&=\sin(y)\\
\end{align*}
Por lo que
\begin{align*}
  f(x,y)= y\sin(x)+x^2+\sin(y)=c
\end{align*}
\item Resuelva la siguiente ecuación diferencial de primer orden
\[x^2y^2 dx+(x^3y+y+3)dy=0\]
\begin{sol}Consideramos $M(x,y)=x^2y^2$ y $N(x,y)=x^3y+y+3$.
\end{sol}
...
\begin{sol}Reescribiendo, obntenemos una ecuación de Bernoulli con $n=-1$
\end{sol}
...
\begin{sol}Podemos hacer el cambio de variable $z=x+y$, luego $\dfrac{dz}{dx}=1+\dfrac{dy}{dx}$. Reemplazando
\end{sol}
\begin{align*}
