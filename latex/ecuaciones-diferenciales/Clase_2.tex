...
\begin{defin}Consideremos una ecuación  en su forma diferencial
\end{defin}
...
\begin{ejem}Consideramos la ecuación diferencial $(2xy^2+1)dx + 2x^2ydy=0	$, entonces la función  $f(x,y)=x^2y^2+x$ cumple que $f_x= 2xy^2+1$ y $f_y= 2x^2y$ \\
\end{ejem}
...
\begin{prop}Supongamos que $M(x,y)$ y $N(x,y)$ son continuas y tienen primeras derivadas parciales en una región $D \subset \mathbb{R}^2$. Entonces una condición necesaria y suficiente para que $M(x,y)dx+N(x,y)dy=0$ sea una EDO exacta es que
\end{prop}
...
\begin{ejem}Sea $(1+e^xy+xe^xy)dx+(xe^x+2)dy=0$. Identifique $M(x,y)$ y $N(x,y)$ y verifique si es exacta. \\
\end{ejem}

\begin{sol}En este caso $M(x,y)= 1+e^xy+xe^xy$ y $N(x,y)= xe^x+2$. Notamos que
\end{sol}
...
\begin{ejer}Resolver $(2x^2+6xy^2)dx+(6x^2y+4y^3)dy=0$\\
\end{ejer}

\begin{sol}Definimos $M(x,y)= 2x^2+6xy^2$ y $N(x,y)= 6x^2y+4y^3$. Verificamos primero la condición de ecuaciones diferenciales para asegurarnos de que no perdemos el tiempo, tenemos que
\end{sol}
...
\begin{ejer}Resolver $y'=\displaystyle \frac{(x+y)^2}{1-2xy-x^2}$ \\
\end{ejer}
...
\begin{rmk}Al termino  $\displaystyle e^{ \int a(x)dx}$ se le conoce como \textbf{factor integrante} y en general es un término que al multiplicar toda la ecuación diferencial por él nos da una ecuación diferencial conocida. \\
\end{rmk}
...
\begin{ejer}Resolver la EDO $\displaystyle y' + \frac{x}{x^2+1} y = 0$ \\
\end{ejer}


\begin{sol}Reconocemos que tenemos una ecuación lineal donde $b(x)=0$ y $a(x)= \displaystyle \frac{x}{x^2+1}$ , la solución viene dada por
\end{sol}
...
\begin{ejer}Resolver la EDO $\displaystyle y' = y\tan(x)+\cos(x)$ \\
\end{ejer}


\begin{sol}Reconocemos que tenemos una ecuación lineal donde $a(x)=-\tan(x)$ y $b(x)= \cos(x)$ , la solución viene dada por
\end{sol}
...
&= \sec(x)\left(  \frac{x}{2} + \frac{\sin(2x)}{4}+c  \right)\\
\end{align*}
