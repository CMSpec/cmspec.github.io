...
\begin{ejem}Tomamos la ecuación diferencial $xy''-3y\sin(x)-y''2x^2+1=0$, en este caso, podemos despejar $y''$ y queda
\end{ejem}
...
\begin{rmk}En este caso fue posible despejar $y''$, pero en general esto no siempre se puede hacer. \\  \ \\
\end{rmk}

\begin{defin}Dada la ecuación diferencial $F(x,y,y',...,y^{(n)})=0$, decimos que $n$ es el \textbf{orden de la ecuación diferencial}, es decir, el mayor índice de la derivada que aparece en la ecuación diferencial. \\
\end{defin}

\begin{ejem}La ecuación diferencial $\displaystyle (y'')^5+3 y''-2\frac{y}{x^2}=0$ tiene orden 2. Por otra parte, la ecuación diferencial dada por $\displaystyle yy'''-4x\ln(y'')=0$ tiene orden 3. \\
\end{ejem}

\begin{rmk}No hay que confundir el orden de la ecuación diferencial con el grado del exponente de $y''$. \\
\end{rmk}




\begin{defin}Una ecuación diferencial $F(x,y,y',...,y^{(n)})=0$ se dice \textbf{Lineal} si $F$ es lineal respecto a $y,y',... , y^{n}$ \\
\end{defin}

\begin{ejem}La EDO $\displaystyle y''-\frac{2}{x^2} y= 0$ es lineal. Por otra parte, la EDO $\displaystyle (y'')^2-\frac{2}{x^2} y= 0$ No es lineal. \\ \\
\end{ejem}
...
\begin{defin}Dada una ecuación diferencial $F(x,y,y',... ,y^{(n)})=0$, se dice que una función continua $\phi$ definida sobre un intervalo $I$ \textbf{es solución de  la ecuación diferencial} si posee por lo menos $n$ derivadas continuas y además
\end{defin}
...
\begin{ejem}Dada la EDO $\displaystyle y''-\frac{2}{x^2}y =0$, se tiene que $\phi(x)= \displaystyle x^2-\frac{1}{x}$ es una solución. Otro ejemplo es la ecuación diferencial dada por $\displaystyle \frac{dy}{dx} =\frac{3x^2}{2y}$, en este caso se tiene que $y^2-x^3+8=0$ es solución. \\
\end{ejem}
...
\begin{defin}Una \textit{Solución explícita} de una EDO, es una solución de la forma $y = \phi(x)$. Es decir, la variable $y$ está despejada sólo en términos de $x$. \\
\end{defin}


\begin{ejem}$y= xe^x$ es solución explícita de la EDO $y''-2y'+y=0$. \\
\end{ejem}




\begin{defin}Una \textit{Solución implícita} de una EDO, es una solución de la forma $f(x,y) = 0$. Es decir, la variable  $y$ no se encuentra despejada sólo  en términos de $x$. \\
\end{defin}


\begin{ejem}$x^2+y^2-25=0$ es solución implícita de la EDO $\displaystyle \frac{dy}{dx}= -\frac{x}{y}$. Porque $y$ no está despejada respecto a $x$\\
\end{ejem}


\begin{rmk}No siempre es posible obtener una solución explícita de una ecuación diferencial porque no siempre se puede despejar $y$ respecto a $x$. \\
\end{rmk}


\begin{rmk}En el ejemplo 2 es posible obtener una solución explícita, las soluciones serían
\end{rmk}
...
\begin{rmk}La solución general en este caso es de la forma $y= \sin(x) +c$, esta escritura en realidad representa un conjunto infinito de soluciones posibles, esto quiere decir que cualquier valor que escribamos para $c$ es una solución diferente, es decir, hay una cantidad infinita de soluciones, una por cada valor que le asignemos a $c$, de esa forma podemos encontrar todas las soluciones de $y'=\cos(x)$. \\
\end{rmk}


\begin{defin}La solución general de una EDO corresponde a encontrar todas las posibles soluciones de la EDO. \\
\end{defin}

\begin{rmk}Resolver una ecuación diferencial significa encontrar la solución general de la EDO, no una solución particular.
\end{rmk}
...
\begin{defin}Un problema de valores iniciales (PVI) consiste en una EDO $F(x,y,y',....,y^{(n)})=0$ y $n$ puntos $(x_0,y_0)$,$(x_1,y_1)$,... ,$(x_{n-1},y_{n-1})$ sujeto a las restricciones
\end{defin}
...
\begin{thm}[Existencia y Unicidad]Sea $D$ un conjunto abierto en el plano y $f: D \to \mathbb{R}$ función continua. Supongamos que $f$ tiene derivadas parciales respecto a $y$ en $D$ y además $\frac{\partial f}{\partial y}$ es continua sobre $D$. Sea $(x_0,y_0) \in D$, entonces la ecuación diferencial
\end{thm}
...
\begin{ejem}Una solución de la  EDO $y'=y$ es $y=e^x$, en general toda función de la forma $y=ce^x$ con $c$ constante será solución. Pero, si agregamos la condición inicial $y(0)=5$, entonces la única solución posible es $y=5e^x$. \\
\end{ejem}

\begin{ejem}Una solución general de la  EDO $y''+y'-2y=0$ está dada por $y(x)=c_1e^x+c_2e^{-2x}$, donde $c_1$ y $c_2$ son constantes (Demuéstrelo). Pero, si agregamos las condiciones iniciales $y(0)=2$ e $y'(0)=1$, entonces la única solución posible es $y(x)= \frac{5}{3}e^{x}+\frac{1}{3}e^{-2x}$. \\
\end{ejem}

Ahora vamos a resolver un ejercicio simple. \\


\begin{ejer}Resuelva el PVI dado por
\end{ejer}
...
\begin{sol}Este tipo de ecuaciones $y^{(n)} = f(x)$ siempre se pueden resolver simplemente integrado múltiples veces, pero tenemos que tener el cuidado de que cada vez que integramos la contante que sale es distinta, para marcar esa diferencia usamos distintos sub-índices.
\end{sol}
...
\begin{defin}Supongamos que una ecuación diferencial tiene forma
\end{defin}
...
\begin{ejem}Las siguientes ecuaciones son ejemplos de ecuaciones de variables
\end{ejem}
...
\begin{rmk}Al resolver las integrales sólo hay que poner una constante $c$, nunca dos. \\
\end{rmk}


\begin{ejem}La ecuación $y'=y$ es de variables separables, en este caso $h(y)=y$ y $g(x)=1$. Tenemos $\displaystyle \frac{dy}{y}=dx$, luego
\end{ejem}
...
\begin{ejer}Resolver $\displaystyle \frac{dx}{dt} = 4(x^2+1)$ de tal forma que $x(\pi/4)=0$\\
\end{ejer}

\begin{sol}La ecuación es equivalente a
\end{sol}
...
\begin{ejer}Encontrar la solución explícita de la EDO $\displaystyle \sin(3x) +2y\cos^3(3x)\frac{dy}{dx}=0$\\
\end{ejer}
...
\begin{align*}
\displaystyle 2ydy &=-\frac{\sin(3x)}{\cos^3(3x)}dx \\
&=-\tan(3x)\sec^2(3x)dx
\end{align*}
