...
\begin{defin}Dada una función $f:\mathbb{R} \to \mathbb{C}$ función periódica de periodo $T$, se define la \textbf{serie de  Fourier} de $f$ evaluada en $t$, como
\end{defin}
...
\begin{thm}Sea $f: \mathbb{R} \to \mathbb{C}$ una función periódica con periodo $T$ derivable a trozos en $[0,T]$. Entonces se verifica que \\
\end{thm}
...
\begin{rmk}Vamos a saltar todos los detalles y demostraciones, ya que no forman parte del propósito de este curso. \\
\end{rmk}
...
\begin{ejem}Considere la función $f(t) = t$ para $0\leq t < 1$ y $f(t+1)=f(t)$, es decir, una función periódica de periodo 1. Vamos a calcular su serie de Fourier, tenemos que
\end{ejem}
...
Por otra parte, recordando que $\sin(2\pi n) =0$ y $\cos(2\pi n) =1$ para $ n\in \mathbb{N}$, se tiene
...
\begin{ejer}Demuestre que la serie de Fourier de la función $f(x)=x$ en el intervalo $(-2,2)$ viene dada por
\end{ejer}

\begin{align*}
    \sum_{n=1}^\infty \frac{(-1)^{n+1}}{n}\sin \left( \frac{n\pi}{2} x\right)
\end{align*}
...
\begin{ejer}Demuestre que la serie de Fourier de la función dada por $f(t)=t^2$ en el intervalo $[-\pi,\pi)$ es
\end{ejer}
...
\begin{ejem}La ecuación diferencial dada por
\end{ejem}
...
\begin{ejem}Consideremos la EDP dada por $u_x+u_t = u$, supongamos que $u= M(x)N(t)$, entonces
\end{ejem}
...
\begin{rmk}En algunos casos es muy importante diferenciar si $k>0$, $k=0$ o $k< 0$, en especial cuando las ecuaciones tienen orden dos, pero en este caso no es necesario. \\ \\
\end{rmk}
...
\begin{ejem}Consideremos el problema de frontera dado por
\end{ejem}
...
\begin{align*}
    M(x) = c_1\cos(\alpha x)+c_2\sin(\alpha x)
\end{align*}
...
\begin{align*}
    ...
    u_3(x,t) &= ce^{-\alpha^2t}(c_1\cos(\alpha x)+c_2\sin(\alpha x))
\end{align*}
...
\begin{align*}
    ...
    u(1,t) &= ce^{-\alpha^2t}(c_2\sin(\alpha)) =0\\
\end{align*}

De la primera igualdad se deduce que $c_1=0$. Notemos que en la última igualdad tenemos $c_2=0$ o bien $\sin(\alpha)=0$, pero $c_2=0$ no nos sirve, por lo tanto, elegimos $\sin(\alpha) =0 $ y entonces se concluye que
...
\begin{align*}
    x -\frac{1}{2}&= \sum_{n=1}^{\infty}c_n\sin(2n\pi x)
\end{align*}
...
\begin{align*}
    - \frac{1}{\pi}\sum_{n=1}^\infty \frac{\sin(2\pi n t)}{n}&= \sum_{n=1}^{\infty}c_n\sin(2n\pi x)
\end{align*}
...
\begin{align*}
    u(x,t) = -\sum_{n=1}^\infty \frac{1}{\pi n}e^{-4n^2\pi^2t}\sin(2n\pi x)
\end{align*}
