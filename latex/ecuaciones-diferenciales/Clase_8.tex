...
\begin{center}
$y_1 =  x^{\alpha}\cos(\beta \ln(x))$ y  $ y_2 =x^{\alpha}\sin(\beta \ln(x))$
\end{center}
...
\begin{ejem}Resolver la ecuación diferencial $x^2y''-3xy'+3y=2x^4e^x$. \\
\end{ejem}


\begin{sol}Primero resolvemos la parte homogenea $x^2y''-3xy'+3y =0$, usando $y=x^m$ obtenemos
\end{sol}
...
Tomamos ahora $f(t) = \sin(at)$, entonces

\begin{align*}
\displaystyle \mathcal{L}[\sin(at)](s) &= \int_0^\infty 1 \cdot \sin(at)e^{-st} dt \\
&= \lim_{A \to \infty} \int_0^A \sin(at)e^{-st}  dt \\
\end{align*}

Vamos a calcular $\displaystyle \int \sin(at)e^{-st}dt$, tomamos $u=\sin(at)$ y $dv=e^{-st}dt$, entonces $du=a\cos(at)dt$ y $v=\displaystyle -\frac{1}{s}e^{-st}$, luego

\begin{align*}
\displaystyle \int \sin(t)e^{-st}dt &= -\frac{\sin(at)e^{-st}}{s}- \int -\frac{1}{s}e^{-st} a\cos(at)dt \\
&= -\frac{\sin(at)e^{-st}}{s}+ \frac{a}{s} \int \cos(at)e^{-st} dt\\
\end{align*}

Vamos a calcular $\displaystyle \int \cos(at)e^{-st}dt$, tomamos $u=\cos(at)$ y $dv=e^{-st}dt$, entonces $du=-a\sin(at)dt$ y $v=\displaystyle -\frac{1}{s}e^{-st}$, luego



\begin{align*}
\displaystyle \int \cos(at)e^{-st}dt &= -\frac{\cos(at)e^{-st}}{s}- \int (-\sin(at))\frac{-1}{s} e^{-st} adt \\
&= -\frac{\cos(at)e^{-st}}{s}- \frac{a}{s} \int \sin(at)e^{-st} dt\\
\end{align*}

Reemplazando

\begin{align*}
\displaystyle \int \sin(t)e^{-st}dt &= -\frac{\cos(at)e^{-st}}{s}- \int -\frac{1}{s}e^{-st} a\cos(at)dt \\
&= -\frac{\sin(at)e^{-st}}{s}+ \frac{a}{s} \left( -\frac{\cos(at)e^{-st}}{s}- \frac{a}{s} \int \sin(at)e^{-st} dt \right)  \\
&= -\frac{\sin(at)e^{-st}}{s}+  -\frac{a\cos(at)e^{-st}}{s^2}- \frac{a^2}{s^2} \int \sin(at)e^{-st} dt \\
\end{align*}

Ahora sumando $\displaystyle \frac{a^2}{s^2} \int \sin(at)e^{-st} dt $ a ambos lados obtenemos

\begin{align*}
\displaystyle \int \sin(t)e^{-st}dt + \frac{a^2}{s^2} \int \sin(at)e^{-st} dt &= -\frac{\sin(at)e^{-st}}{s} -\frac{a\cos(at)e^{-st}}{s^2} \\
\end{align*}
...
\begin{align*}
\displaystyle  \int \sin(at)e^{-st} dt  \left( 1+ \frac{a^2}{s^2}\right) &= -\frac{\sin(at)e^{-st}}{s} -\frac{a\cos(at)e^{-st}}{s^2} \\
\end{align*}

\begin{align*}
\displaystyle  \int \sin(at)e^{-st} dt  \left( \frac{a^2+s^2}{s^2}\right) &= -\frac{\sin(at)e^{-st}}{s}  -\frac{a\cos(at)e^{-st}}{s^2} \\
\end{align*}


\begin{align*}
\displaystyle  \int \sin(at)e^{-st} dt &= \left( -\frac{\sin(at)e^{-st}}{s} -\frac{a\cos(at)e^{-st}}{s^2} \right) \frac{s^2}{a^2+s^2}\\
\end{align*}
...
\begin{align*}
\displaystyle \mathcal{L}[\sin(t)](s) &= \lim_{A \to \infty} \int_0^A \sin(t)e^{-st}  dt \\
&= \lim_{A \to \infty} \left( \left( -\frac{\sin(at)e^{-st}}{s} -\frac{a\cos(at)e^{-st}}{s^2} \right) \frac{s^2}{a^2+s^2}\right) \bigg|_0^A \\
&= \frac{a}{a^2+s^2}
\end{align*}
...
\begin{align*}
\displaystyle \mathcal{L}[\sin(at)](s) &= \frac{a}{a^2+s^2}
\end{align*}
...
\begin{align*}
\displaystyle \mathcal{L}[\sin(at)](s) &= \frac{a}{a^2+s^2}, \forall s>0
\end{align*}
...
\begin{thm}La transformada de Laplace es una transformación lineal. \\
\end{thm}

Esto significa que la transformada de laplace separa sumas y saca afuera las contantes. \\

\begin{ejem}Calcular la transformada de Laplace de $f(t)=1+4t^4+\cos(2t)$. \\
\end{ejem}
...
\begin{defin}Una función $f$ se dice de orden exponencial si existe $\alpha \in \mathbb{R}$ y $M >0$ tal que
\end{defin}
...
\begin{ejem}La función $\cos(x)$ es de orden exponencial pues $|\cos(x)| \leq 1 = 1 \cdot e^{0 t}$, por otra parte la función exponencial $e^{ax}$ también es de orden exponencial pues $|e^{ax}| =e^{ax} \leq 1 \cdot e^{ax}$. Se tiene además que cualquier polinomio es de orden exponencial. \\
\end{ejem}



\begin{defin}Diremos que una función es continua por trozos si tiene a lo más un número finito de discontinuidades. \\
\end{defin}

Con estas definiciones tenemos el siguiente teorema. \\

\begin{thm}Sea $f$ función continua a trozos en $(0,\infty)$ y de orden exponencial, entonces existe $\mathcal{L}[f]$. Además, si $g$ es otra función continua a trozos de orden exponecial tal que $\mathcal{L}[f]=\mathcal{L}[g]$, entonces se tiene que $f=g$ \\ \\
\end{thm}
...
\begin{center}
$\displaystyle \mathcal{L}^{-1}\left[ \frac{k}{s^2+k^2} \right] (t) = \sin(kt)$
\end{center}
...
\begin{ejem}Calcular $\displaystyle \mathcal{L}^{-1}\left[ \frac{s+1}{s^2(s+2)} \right] $. \\
\end{ejem}

\begin{sol}Tenemos que usar
\end{sol}
...
\begin{ejem}Calcular $\displaystyle \mathcal{L}^{-1}\left[ \frac{3s+4}{s^2+7} \right] $. \\
\end{ejem}

\begin{sol}En este caso no se usan fracciones parciales porque el denominador no se puede factorizar
\end{sol}
...
\begin{center}
$\displaystyle \mathcal{L}^{-1}\left[ \frac{3s+4}{s^2+7} \right] = 3 \cos(\sqrt{7}t) + \frac{4}{\sqrt{7}}\sin(\sqrt{7}t) $
\end{center}
...
\begin{thm}Supongamos que $f$ es continua en $(0,+\infty)$ y que además $f$ es de orden exponencial. Además supongamos que $f'$ es continua por tramos y también de orden exponencial. Entonces
\end{thm}
...
\begin{ejem}Caclular $\displaystyle \mathcal{L}\left[ \sin^2(at) \right] $. \\
\end{ejem}

notemos que $\sin^2(a \cdot 0) =0$, además

\begin{center}
$(\sin^2(at))' = 2a\cos(at)\sin(at) = a\cos(2at)$
\end{center}
...
\begin{center}
$ \displaystyle \mathcal{L}\left[ (\sin^2(at))'\right]  = a\mathcal{L}\left[ \cos(2at)\right] = \frac{2a^2}{s^2+4a^2} $
\end{center}

Por lo tanto

\begin{center}
$ \displaystyle \mathcal{L}\left[\sin^2(at)\right]  = \frac{2a^2}{s(s^2+4a^2)} $
\end{center}
...
\begin{prop}Sea $f$ una función de orden exponencial, entonces
\end{prop}
