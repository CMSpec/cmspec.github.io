...
2- Si $g(x)=\alpha\sin(\omega x)$ o bien $g(x)=\alpha\cos(\omega x)$ con $\alpha \in \mathbb{R}$, entonces hay que asumir $y_p= A\sin(\omega x)+B\cos(\omega x)$. \\
...
\begin{sol}Primero resolvemos la homogenea $y''+y'=0$, tomando $y=e^{mx}$ obtenemos $m^2+1=0$, las soluciones son $m_1=i$ y $m_2=-i$, la solución es  $y_1 = \cos(x)$ y $y_2 =\sin(x)$. \\
\end{sol}

Suponemos que la solución particular es de la forma $y_p= c_1(x)y_1 +c_2(x)y_2$. Primero calculamos $W(y_1,y_1)$, entonces

\begin{center}
$\displaystyle W(y_1,y_2) = y_1y_2'-y_2y_1' = \cos^2(x)+\sin^2(x) =1$
\end{center}
...
\begin{center}
$c_1(x) = \displaystyle \int -\sin(x) \frac{1}{\cos^2(x)} dx = \int -\tan(x)\sec(x)dx = -\sec(x)$
\end{center}
...
\begin{center}
$y= \displaystyle c_1\cos(x) +c_2\sin(x) - \cos(x)\sec(x)+ \sin(x) \ln(\sec(x)+\tan(x)) $
\end{center}
