...
\begin{defin}Se dice que \( f \) es \textbf{Riemann integrable} en \([a,b]\) si:
\end{defin}
...
\begin{defin}Se dice que \( f \) es \textbf{Riemann integrable} en \( R \) si:
\end{defin}
...
\begin{prop}Sea $f: R \to \mathbb{R}$ función acotada y continua en todo $R$ salvo a lo más en un número finito de curvas suaves (diferenciables), entonces $f$ es Riemann integrable \\
\end{prop}
...
\begin{prop}[Propiedades de las integrales dobles]Sean $f,g: R \to \mathbb{R}$ integrables en $R$ y sea $k \in \mathbb{R}$. Entonces \\
\end{prop}
...
\begin{thm}[Fubini para rectángulos]Sea $f: R \to \mathbb{R}$ integrable, donde $R =[a,b]\times [c,d]$, entonces:
\end{thm}
...
\begin{ejem}Calcular la integral doble de la función $f(x,y)=x^2y$ sobre el rectángulo $R=[0,2] \times [1,3]$ .\\
\end{ejem}

\begin{sol}Queremos calcular
\end{sol}
...
\begin{rmk}Si hubieramos calculado primero la integral respecto a $y$ y después respecto a $x$ el resultado sería el mismo. \\
\end{rmk}
...
\begin{sol}$\displaystyle \frac{65}{3}$
\end{sol}
...
\begin{ejer}Calcule $\displaystyle \iint_D x^2+y^2 dA$, donde $D$ es la región limitada por $y=x^2$ e $y=\sqrt{x}$ \\
\end{ejer}

\begin{sol}Para hacer estos ejercicios es muy útil dibujar las regiones de integración, en este caso la región es la siguiente
\end{sol}
...
\begin{ejer}Calcule $\displaystyle \iint_D x+2y dA$, donde $D$ es la región limitada por $y=2x^2$ e $y=1+x^2$ \\
\end{ejer}




\begin{ejer}Calcule la sigueinte integral invirtiendo el orden de integración
\end{ejer}
...
\begin{ejer}Calcule la siguiente integral invirtiendo el orden de integración
\end{ejer}
