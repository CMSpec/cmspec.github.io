...
\begin{ejem}La función dada por
\end{ejem}
...
\begin{ejem}Por ejemplo podemos tomar $f: \mathbb{R}^2 \to \mathbb{R}$ dada por $f(x,y) =\sqrt{x^2-y^2}$, tenemos que esta funación está definida cuando $x^2 \geq y^2$ o bien $|x| \geq |y|$ por lo tanto $ -|x| \leq y \leq |x|$. Luego
\end{ejem}
...
\begin{ejem}Podemos tomar $f: \mathbb{R}^3 \to \mathbb{R}$ dada por $f(x,y,z) = x^2+y^2z^3 $, en este caso la función está bien definida siempre por lo tanto
\end{ejem}
...
\begin{ejem}Sea $f: \mathbb{R}^2 \to \mathbb{R}$ dada por $ \displaystyle f(x,y) = \frac{1}{x^2-y^2}$, en este caso la no función está bien definida cuando  $x^2=y^2$, es decir $|x|=|y|$, por lo tanto
\end{ejem}
...
\begin{ejem}Sea $f: \mathbb{R}^2 \to \mathbb{R}^2$ dada por $ \displaystyle f(x,y) = \left(  \frac{1}{x^2-y^2}, \frac{1}{x} \right) $. En este caso la primera coordenada es función es $ \frac{1}{x^2-y^2}$ no está definida cuando  $x^2=y^2$, es decir $|x|=|y|$. La segunda coordenada  tampoco está bien definida si $x=0$, por lo tanto
\end{ejem}
...
\begin{ejer}Determine el gráfico del dominio de la función
\end{ejer}
...
\begin{ejem}Sea $f(x,y) = x^2 +y^2$, para $a \in \mathbb{R}$ la curva de nivel es $a= x^2+y^2$, notemos que $a$ solo puede ser mayor o igual que cero pues nunca es negativo, por lo tanto la superficie consiste en circulos de radio $\sqrt{a}$ a lo largo del eje $z$\\
\end{ejem}
...
\begin{ejem}La curva que define al circulo $x^2+y^2 = r^2$, se puede parametrizar como
\end{ejem}


\begin{align*}
f(t) = \left( r\cos(t), r\sin(t) \right)
\end{align*}
...
\begin{align*}
f(t) = \left( r\cos(t)+a, r\sin(t)+b \right)
\end{align*}


\begin{ejem}Una  elipse de eje mayor $a$, eje menor $b$ y centrada en el punto $(x_0,y_0) $ se puede parametrizar como
\end{ejem}


\begin{align*}
f(t) = \left( a\cos(t)+x_0, b\sin(t)+y_0 \right)
\end{align*}

\begin{ejem}Una  hipérbola de ecuación
\end{ejem}
...
\begin{defin}Sea $A \subseteq \mathbb{R}^n$, se dice que $A$ es abierto, si para todo $x \in A$ existe un $r > 0$, tal que $B_r(x) \subset A$/ \\
\end{defin}

\begin{defin}Sea $C \subseteq \mathbb{R}^n$, se dice que $C$ es cerrado si $C^c$ es abierto. \\
\end{defin}
...
\begin{defin}Sea $\mathbb{R}^m \to \mathbb{R}^n$ y $a \in \mathbb{R}^m$, decimos que $\displaystyle \lim_{x \to a} f(x) = L$ si y solo si $\forall$ $\epsilon > 0$ $\exists$ $\delta >0$ tal que
\end{defin}
...
\begin{ejem}Sea $f(x,y) = (3x,2y)$, vamos a demostrar por definici\'on que
\end{ejem}
...
\begin{ejem}Si $f(x) = \displaystyle \left( \frac{x^2-1}{x-1}, \frac{\sin(x)}{x}, \frac{x}{\sqrt{x}}\right)$, entonces
\end{ejem}

\begin{align*}
    \lim_{x \to 0} f(x) &= \left(\lim_{x \to 0} \frac{x^2-1}{x-1},  \lim_{x \to 0} \frac{\sin(x)}{x}, \lim_{x \to 0} \frac{x}{\sqrt{x}} \right) \\
    &= (2,1,0)
\end{align*}

Los límites de curvas parametrizadas se calculan aplicando los límites de funciones de una variable a cada componente. \\


\begin{ejer}Determine los siguientes l\'imites\\
\end{ejer}
