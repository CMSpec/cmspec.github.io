...
\begin{thm}[Cambio de variable]Sea $T : R_{uv} \subset \mathbb{R}^2 \to R_{xy}\subset \mathbb{R}^2$ una transformaci\'on inyectiva y de clase $C^1$ en el plano $uv$ en el plano $xy$ tal que $T (u, v ) = (x (u, v ) , y (u, v ))$. Si $f : R_{xy} \to \mathbb{R}$ es Riemann integrable, entonces:
\end{thm}
...
\begin{ejer}Calcule el valor de $\iint_{R} x+y+1dA$ donde $R$ es la regi\'on acotada por las gr\'aficas de $y-x=1$, $y-x=-1$, $y+x=1$ e $y+x=2$ \\
\end{ejer}

\begin{sol}Este ejercicio es muy dif\'icil de hacer sin usar cambio de variable, por suerte podemos usar el cambio de variable para hacerlo de forma sencilla. \\
\end{sol}
...
\begin{ejer}Calcule el valor de $\displaystyle \iint_{R} \sqrt{\frac{x}{y}}+ \sqrt{xy} dA$ donde $R$ es la regi\'on acotada por las gr\'aficas de $xy=1$, $yx=9$, $y=x$ e $y=4x$ \\
\end{ejer}

\begin{sol}Es claro que uno de los cambios de variable que queremos hacer es $u=xy$, el otro cambio no est\'a tan claro, pero si dividimos por que las segundas dos condiciones, obtenemos que $y/x= 1$ e $y/x=4$, por lo tanto el segundo cambio de variable conveniente es $v= y/x$. \\
\end{sol}
...
\begin{sol}Aqu\'i nos conviene pasar a coordenadas polares, primero hay que entender la regi\'on de integraci\'on, tenemos que $x$ var\'ia entre 0 y 1 mientras que $y$ var\'ia entre la recta $y=0$ y la curva $y= \sqrt{1-x^2}$, esto corresponde a un cuarto de circunferencia de radio 1
\end{sol}
...
\begin{sol}Tenemos que $x$ var\'ia entre 0 y 3 mientras que $y$ var\'ia entre la recta $y=0$ y la curva $y= \sqrt{9-x^2}$, esto corresponde a un cuarto de circunferencia de radio 3. \\
\end{sol}
...
\begin{ejer}Determine la masa y el centro de masa  de la l\'amina que ocupa la regi\'on $D$ del primer cuadrante  acotada por $y=x^2$ y la recta $y=1$ y que tiene funci\'on de densidad $\rho(x,y) = xy$ \\
\end{ejer}

\begin{sol}La masa ser\'a
\end{sol}
...
\begin{ejer}Determine el momento de inercia del eje $X$ de la l\'amina correspondiente a la regi\'on parab\'olica en el primer cuadrante dada por  $0 \leq y \leq 4-x^2$ si $\rho(x,y) = xy$ \\
\end{ejer}

\begin{sol}El momento de inercia pedido viene dado por
\end{sol}
...
\begin{ejem}Calcular el \'area de la proci\'on del plano $z=2-x-y$ que esta situada encima del ci\'rculo $x^2+y^2 \leq 1$ en el primer cuadrante \\
\end{ejem}

\begin{sol}Dado que \( z = f(x,y)= 2 - x - y \), tenemos
\end{sol}
