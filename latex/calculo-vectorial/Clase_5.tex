...
Extendemos los conceptos de extremo local y punto crítico a funciones de varias variables \\

\begin{defin}Sea $f: A \subseteq \mathbb{R}^n \to \mathbb{R}$, donde $A$ es un conjunto abierto, se dice que $x_0 \in A$ es un \textbf{mínimo local} si existe $r> 0$ tal que para todo $x \in B(x_0,r) \subseteq A$ se tiene $f(x_0) \leq f(x)$. De forma análoga se dice que   $x_0 \in A$ es un \textbf{máximo local} si existe $r> 0$ tal que para todo $x \in B(x_0,r) \subseteq A$ se tiene $f(x_0) \geq f(x)$.  \\
\end{defin}
...
\begin{defin}Sea $f: A \subseteq \mathbb{R}^n \to \mathbb{R}$, donde $A$ es un conjunto abierto, se dice que $x_0 \in A$ es un \textbf{punto crítico} de $f$ si y sólo si $D f(x_0) = 0$, es decir, si el diferencial de $f$ en $x_0$ es igual a $0$. \\
\end{defin}
...
\begin{ejem}Hallar los puntos críticos de $f(x,y) = x^2+2y^2-x+2y $ \\
\end{ejem}
...
\begin{thm}$f: A \subseteq \mathbb{R}^n \to \mathbb{R}$, donde $A$ es un conjunto abierto y $x_0 \in A$, si $x_0$ es un extremo local, entonces es un punto cr\'itico
\end{thm}
...
\begin{rmk}No todos los puntos críticos corresponden a extremos locales.\\
\end{rmk}

\begin{defin}Sea $f: A \subseteq \mathbb{R}^n \to \mathbb{R}$, donde $A$ es un conjunto abierto y sea $x_0 \in A$ es un punto crítico, se dice que $x_0$ es un \textbf{punto silla} si para toda bola abierta $B(x_0,r) \subseteq A$ se tiene que existen puntos $x,y \in B(x_0,r)$ tales que $f(x_0) < f(x)$ y $f(y) < f(x_0)$. \\
\end{defin}
...
\begin{defin}Sea $f(x,y)$ función diferenciable en dos variables y sea $x_0$ un punto en el dominio de $f$, definimos la matriz Hessiana como
\end{defin}
...
\begin{thm}Sea $x_0 \in \mathbb{R}^2$ y $f$ función diferenciable con dominio en una bola abierta. Supongamos que $x_0$ es un punto crítico de $f$. Entonces \\
\end{thm}
...
\begin{ejem}Hallar y clasificar los puntos críticos de la función $f(x,y) =x^3+y^3-3xy$ \\
\end{ejem}
...
\begin{ejer}Determine los valores extremos de $f(x,y) = e^x\cos(y)$ \\
\end{ejer}

\begin{ejer}Determine la distancia m\'inima entre el origen y la superficie $z^2=x^2y+4$\\
\end{ejer}
...
\begin{ejem}Encontrar los valores extremos de la función $f(x,y)= y^2-x^2$ sobre la elipse $x^2+4y^2=4$ \\
\end{ejem}

\begin{sol}En este caso la restricción es $g(x,y)= x^2+4y^2-4$ y hay que resolver el sistema
\end{sol}
...
\begin{ejer}Determine  los extremos de la funci\'on $f(x,y) = e^{xy}$ con la restricci\'on $x^2+y^2=16$ \\
\end{ejer}

\begin{ejer}Determine  los extremos de la funci\'on $f(x,y,z) = x+y+z$ con la restricci\'on $\frac{1}{x}+\frac{1}{y}+\frac{1}{z} = 1$ \\
\end{ejer}
