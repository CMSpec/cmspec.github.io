...
\textbf{Teorema 1: } Sea $\mathbb{R}^m \to \mathbb{R}^n$ y $a \in \mathbb{R}^m$, en caso de que exista $\displaystyle \lim_{x \to a} f(x) = L$ entonces es único \\ \\




\textbf{Teorema 2: } Sea $\mathbb{R}^m \to \mathbb{R}^n$ y $a \in \mathbb{R}^m$, en caso de que exista $\displaystyle \lim_{x \to a} f(x) = L$ entonces dada cualquier curva parametrizada $h(t)$ y $t_0 \in \mathbb{R}$, tal que $ a=h(t_0) $, se debe tener \\
...
\textbf{Ejemplo 1: } Vamos demostrar que \\
...
\textbf{Ejemplo 2: } Vamos demostrar que \\
...
\textbf{Ejemplo 3: } En caso de que no haya ninguna indeterminación el cálculo es muy fácil \\
...
\textbf{Teorema del Sandwich:} Sea $f: \mathbb{R}^n \to \mathbb{R}$ y sean $g(x)$ y $h(x)$ funciones tales que $g(x) \leq f(x) \leq h(x)$ $\forall x \in \mathbb{R}^n$. Supongamos adem\'as que $\displaystyle \lim_{x \to a} g(x) = \lim_{x \to a} h(x) = L$, es decir, los l\'imites existen y son iguales, entonces $\displaystyle\lim_{x \to a} f(x)$ existe y

\begin{align*}
    \lim_{x \to a} f(x) = L
\end{align*}

\textbf{Ejemplo:} Determinar

\begin{align*}
    \lim_{(x,y) \to (0,0)} \frac{|\sin(x)\sin(y)|}{|x|+|y|}
\end{align*}

\textbf{Respuesta:} Recordamos que $|\sin(x)| \leq x$, luego
...
\textbf{Ejercicio para el alumno: }Usando el teorema del sandwich, determine el siguiente l\'imite
...
\textbf{Ejemplo 1: } Vamos a calcular \\
...
\textbf{Definición:} Dada una función $f: \mathbb{R}^n \to \mathbb{R}$ Se define la derivada direccional  en el punto $a$ y en la dirección $u $, como
...
\textbf{Ejemplo:} Sea $f(x,y) = x^2+y^2$ y $u=(1,1)$, calculemos $D_u f (1,0)$, tenemos que
