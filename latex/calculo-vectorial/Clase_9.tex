...
\textbf{Definición: }Se dice que \( f \) es \textbf{Riemann integrable} en \( B \) si:
...
\textbf{Proposición:} Sea $f: B \to \mathbb{R}$ función acotada y continua en todo $R$ salvo a lo más en un número finito de superficies suaves (diferenciables), entonces $f$ es Riemann integrable \\

\textbf{Proposición (Propiedades de las integrales triples):} Sean $f,g: B \to \mathbb{R}$ integrables en $B$ y sea $k \in \mathbb{R}$. Entonces \\
...
\textbf{Teorema (Fufini para rectángulos):} Sea $f: B \to \mathbb{R}$ integrable, donde $B =[a,b]\times [c,d] \times [r,s]$, entonces:
...
\textbf{Observaci\'on: }Una aplicaci\'on importante de las integrales triples es que si consideramos $f(x,y,z)= 1$, entonces $\displaystyle \iint_E dV$ representa el volumnen de

\newpage


\textbf{Ejercicio:} Sea $E$ la regi\'on acotada por la gr\'afica del plano $x+y+z=1$ en el primer octante. Calcule
...
\textbf{Respuesta:} Primero calculamos las intersecciones con los ejes
...
\textbf{Teorema: } Sea $T: E_{uvw} \to \mathbb{R}^3$ transformaci\'on inyectiva, continua y diferenciable del plano $uvw$ en el plano $xyz$ tal que
...
\textbf{Ejemplo 1:} Calcule el volumen del s\'olido acotado por las regiones $z= x^2+y^2$ y $z=2-x^2-y^2$ \\

\textbf{Respuesta:} El gr\'afico corresponde a la sigueinte imagen
...
\textbf{Ejercicio 1:} Calcule el volumen del s\'olido $E$ acotado por lasgr\'aficas de $z=\sqrt{x^2+y^2}$, $z=\sqrt{1-x^2-y^2}$ y $z=\sqrt{9-x^2=y^2}$ \\

\textbf{Respuesta:} La gr\'afica corresponde a  
