...
\begin{ejem}Sean $\overrightarrow{u}=(1,-3,4)$, $\overrightarrow{v}=(-1,2,-12)$ y $\overrightarrow{w} = (2,4,-17)$. Entonces
\end{ejem}
...
\begin{ejem}Sean $\overrightarrow{u}=(1, 3, 5)$, $\overrightarrow{v}=(-8,4,3)$. Entonces
\end{ejem}
...
Esta igualdad se obtiene aplicando el teorema del coseno. Notemos que esto también nos permite extender el concepto de ángulo entre vectores a dimensiones más grandes, en general se tiene que el ángulo entre dos vectores viene dado por
...
\begin{ejem}Sean $\overrightarrow{u} = (5,-2,1)$ y $\overrightarrow{v} = (1,-7,-1) $, luego el producto punto entre los vectores viene dado por
\end{ejem}
...
\begin{ejem}Sean $\overrightarrow{u} = (5,-2,1)$ y $\overrightarrow{v} = (1,-7,-1) $, luego la distancia entre los dos vectores viene dado por
\end{ejem}
...
\begin{ejem}Sean $\overrightarrow{u} = (4,6,3)$ y $\overrightarrow{v} = (1,1,-1) $, luego el ángulo entre estos dos vectores viene dado por
\end{ejem}
...
\begin{defin}Dados dos vectores $\overrightarrow{u}, \overrightarrow{v} \in \mathbb{R}^n$ se dice que son \textbf{ortogonales } o \textbf{perpendiculares} si y sólo sí $\overrightarrow{u} \cdot \overrightarrow{v} = 0$ \\
\end{defin}

\begin{ejem}$\overrightarrow{u}= (-1,1,1)$ y $\overrightarrow{v} = (2,1,1)$, entonces
\end{ejem}
...
\begin{ejem}Tomando $\overrightarrow{u} =(2,5)$ y $\overrightarrow{v} =(3,4) $, entonces
\end{ejem}
...
\begin{center}
$||\overrightarrow{u} \times \overrightarrow{v} || =||\overrightarrow{u}||||\overrightarrow{v}|| \sin(\alpha)$
\end{center}
...
\begin{ejem}Sean $\overrightarrow{u} =(1,1,1)$ y $\overrightarrow{v} = (1,-2,-1) $, entonces
\end{ejem}
...
\begin{defin}un vector $u$ se dice unitario si $|| u || =1$ \\
\end{defin}
...
\textbf{Distancia de un punto a una recta:} La distancia de un punto $A$ a una recta $L: \overrightarrow{p}+ \lambda \overrightarrow{d}$ viene dada por
...
\begin{ejem}Calcular la distancia entre el punto $A=(1,3,5)$ y la recta $L$ dada por $\displaystyle \frac{x-1}{2} = \frac{y+2}{3}  = z $. \\
\end{ejem}

\begin{sol}Primero hay que encontrar el vector posición y el vector director de la recta, se tiene que $\overrightarrow{d}=(2,3,1)$ y  $\overrightarrow{p}=(1,-2,0)$. Entonces
\end{sol}
...
\begin{ejem}Consideramos las rectas  $\displaystyle L_1 :  \frac{x-5}{3} = \frac{y+2}{7}  = z+4 $ y $\displaystyle L_2 :  \frac{x-5}{2} = \frac{y-2}{6}  = \frac{z-1}{4} $. Quremos ver que tipo de rectas tenemos. \\
\end{ejem}

\begin{sol}Tenemos que los vectores directores de cada recta son $\overrightarrow{d}= (3,7,1)$ y $\overrightarrow{v}=(2,6,4)$, como los vectores no son paralelos pues $\displaystyle \frac{3}{2} \neq \frac{7}{6}$. Entonces, es claro que estamos en el caso $2$. Los vectores posición vienen dados por $\overrightarrow{p}=(5,-2,-4)$ y $\overrightarrow{u}=(5,2,1)$. Notamos que
\end{sol}
...
Un plano $\Pi$ esta determinado por dos vectores directores $\overrightarrow{d}$ y $\overrightarrow{v}$ y un vector posición $\overrightarrow{p}$ y se puede escribir de la forma
...
\begin{ejem}Consideramos el plano $\Pi$ generado por los vectores $\overrightarrow{d}=(1,2,3)$ y $\overrightarrow{v}=(-3,4,7)$ y el vector posición $\overrightarrow{p}= (1,0,3)$. Entonces el plano $\Pi$ esta dado por
\end{ejem}
...
\begin{ejem}Calcular la ecuación del plano cuyo vector normal es $\overrightarrow{n}=(1,7,-3)$ y que pasa por el punto $(-3,2,4)$.\\
\end{ejem}

\begin{sol}Ya tenemos $A$, $B$ y $C$, estos vienen dados por las coordenadas del vector normal, es decir $A=1$, $B=7$ y $C=-3$, luego la ecuación general es de la forma
\end{sol}
