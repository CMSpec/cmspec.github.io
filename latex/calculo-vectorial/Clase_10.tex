...
\textbf{Definici\'on:} Si $f(x_1,x_2,...,x_n)$ es una funci\'on escalar, es decir, $f: \mathbb{R}^n \to \mathbb{R}$, entonces se define la integral de l\'inea de $f$ sobre $C$ como

\begin{align*}
    \int_C f  ds = \int_\alpha^\beta f(x_1(t),x_2(t),...,x_n(t))||r'(t)|| dt
\end{align*}

\textbf{Definici\'on:} Si $f(x_1,x_2,...,x_n)$ es una funci\'on vectorial, es decir, $f: \mathbb{R}^n \to \mathbb{R}^m$, entonces se define la integral de l\'inea de $f$ sobre $C$ como
...
\textbf{Ejemplo 1:} Calcular la integral de línea del campo escalar
\[
f(x, y) = x^2 + y^2
\]
...
\textbf{Ejemplo 2:} Sea el campo vectorial $F(x, y) = (y, x)$.  
Calcular:
...
\textbf{Respuesta: } Primero hay que parametrizar la curva $C$, tenemos que
...
\textbf{Ejercicio para el alumno: } Sea el campo vectorial
\[
F(x, y) = (x^2 - y,\ xy + 2)
\]
...
\textbf{Ejercicio para el alumno: }  Sea el campo escalar
\[
f(x, y) = x + y
\]
...
\textbf{Definici\'on: }Sea $X$ conjunto, $X$ se dice simplemente conexo si para toda aplicaci\'on continua $f:[0,1] \to X$ tal que $f(0)=f(1)=p$, con $p \in X$, existe una apliaci\'on continua $H: [0,1] \times [0,1] \to X$ tal que $H(s,0) =f(s)$ y $H(s,1)=p$. \\
...
\textbf{Definici\'on: }Sea $F:A \subseteq \mathbb{R}^n \to \mathbb{R}^n$, donde $A$ es un conjunto abierto simplemente conexo , se dice que $F$ es \textbf{conservativo} si existe un campo escalar $f:\mathbb{R}^n \to \mathbb{R}$ tal que
...
\textbf{Proposici\'on:} Supongamos que $F: A \subseteq \mathbb{R}^2 \to \mathbb{R}^2$, con $A$ simplemente  conexo, $F(x,y)=M(x,y)\hat{\imath}+N(x,y)\hat{\jmath}$, entonces $F$ es conservativo  si y solo si
...
\textbf{Definici\'on :} Sea $F: A \subseteq \mathbb{R}^3 \to \mathbb{R}^3$ , $F(x,y,z)=M(x,y,z)\hat{\imath}+N(x,y,z)\hat{\jmath}+C(x,y,z)\hat{k}$, se define el \textbf{rotacional} de $F$ como
...
\textbf{Proposici\'on:} Supongamos que $F: A \subseteq \mathbb{R}^3 \to \mathbb{R}^3$ con $A$ simplemente conexo, $F(x,y,z)=M(x,y,z)\hat{\imath}+N(x,y,z)\hat{\jmath}+C(x,y,z)\hat{k}$, entonces $F$ es conservativo  si y solo si
...
\textbf{Ejemplo 1: } Determinar si el campo $F(x,y) = (6xy^3,9x^2y^2)$ es conservativo o no \\

\textbf{Respuesta: } Tenemos que $M= 6xy^3$ y $ N = 9x^2y^2$, luego
...
\textbf{Ejemplo 2: } Determinar si el campo $F(x,y,z) = (2xyz^3,x^2z^3, 3x^2yz^2)$ es conservativo o no \\

\textbf{Respuesta: } Tenemos que $M= 2xyz^3$, $ N = x^2z^3$ y $C=3x^2yz^2 $, luego
...
\textbf{Teorema:} Sea $F(x,y,z)$ campo conservativo, considere $C$ la curva parametrizada por $\lambda(t)=(tx,ty,tz)$ con $t \in [0,1]$, consideramos
...
\textbf{Ejemplo 1:} Sea $F(x,y) =(x+y^2,2xy)$, demuestre que $F$ es conservativo y luego calcule su funci\'on potencia \\

\textbf{Respuesta: } Notamos que $M_y=N_x= 2y$ por lo tanto el campo es conservativo, la funci\'on potencia ser\'a
...
\textbf{Ejercicio:} Sea $F(x,y,z) =(3y^2z+ye^x,6xyz+e^x,3xy^2)$, demuestre que $F$ es conservativo y luego calcule su funci\'on potencia \\

\textbf{Respuesta:} Tenemos que
...
\textbf{Ejemplo:} Sea $F(x,y)= (2xy+3,x^2+\cos(y))$, usando el teorema fundamental de las integrales de línea, calcule
...
\textbf{Solución:} No parematrizamos la curva, primero vemos que sea conservativo, tenemos que $M= 2xy+3$ y $N=x^2+\cos(y)$, luego $M_y=N_x=2x$, por lo tanto es conservativo, la función potencia será
